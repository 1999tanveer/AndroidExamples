...
class HorseObj(type:Int) : Horse {
    ...
    //, weight:Int, speed:Int
     override fun skill() {
        ...
        println("Horse$type is running at $speed Km/hr Ballee Ballee!!!")
        }
}

class GunObj(type:Int) : Guns {
    var capacity:Int=0
        get(){
            return capacity
        }
...
class BulletObj(type:Int) : Bullets {
    var size:Int =0
        get(){
            return size
        }
...
class SwordObj(type:Int) : Swords {
    var length:Int =0
    get(){
        return length
    }
